...
const SocialLinksPage = () => {
  ...
  const fetchLinks = async () => {
    try {
      setLoading(true);
      const response = await apiService.get("/social-links");
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await apiService.post("/social-links", editRow);
      await fetchLinks();
      setEditRow(null);
    } catch (error) {
  ...
  const tableConfig = [
    ...
    },
    {
      ...
      render: (_, row) =>
        editRow?.user === row.user ? (
          <>
            <CButton size="sm" color="success" className="me-2" onClick={handleSave} disabled={saving}>
