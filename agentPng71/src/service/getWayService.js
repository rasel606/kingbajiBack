...
export const getWayService = {
  searchTransactionsDeposit: async (params = {}) => {
    ...
    const response = await apiService.get('/agent/search_deposit_transactions', params);
    return response; // apiService.get already returns parsed JSON
  },
  searchTransactionsWidthrawal: async (params = {}) => {
    // ✅ use GET with query params
    const response = await apiService.get('/agent/search_Widthrawal_transactions', params);
    return response; // apiService.get already returns parsed JSON
  },
  TransactionsDepositGetways: async (params = {}) => {
    // ✅ use GET with query params
    const response = await apiService.get('/agent/deposit_getWay_list', params);
    return response; // apiService.get already returns parsed JSON
  },
