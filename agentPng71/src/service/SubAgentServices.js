...
export const SubAgentServices = {
  GetUserList: async (params = {}) => {
    ...
    const response = await apiService.get('/agent/get_userList', params);
  ...
  GetSubAgentUserDepositList: async (params = {}) => {
    // ✅ use GET with query params
    const response = await apiService.get('/agent/widthrow_getWay_list', params);
    console.log('response usert', response)
    return response; // apiService.get already returns parsed JSON
  },
  GetAgentList: async (params = {}) => {
    // ✅ use GET with query params
    const response = await apiService.get('/agent/get_agent_List', params);
    console.log('response usert', response)
    return response; // apiService.get already returns parsed JSON
  },
  getUserById: async (userId) => {
    const response = await apiService.get(`/agent/get_users_by_Id/${userId}`);
  ...
  updateUserProfileById: async (userId, data) => {
    ...
    const response = await apiService.put(`/agent/get_users_by_Id_update/${userId}`, data);
    return response.data;
  },

  // Verify email
  verifyEmailForUser: async (userId) => {
    const response = await apiService.post(`/agent/get_users_verify-email/${userId}`);
    return response.data;
  },

  // Verify phone
  verifyPhoneForUser: async (userId) => {
    const response = await apiService.post(`/agent/get_users_verify-phone/${userId}`);
    return response.data;
  },
  changeUserPassword: async (userId, password) => {
    const { data } = await apiService.put(`/Admin/get_users_by_Id_Password_update/${userId}`, { password });
    return data;
  },
