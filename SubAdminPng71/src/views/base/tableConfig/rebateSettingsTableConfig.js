...
export const rebateSettingsTableConfig = ({ onEdit }) => [
    ...
    { key: "rebateTurnoverPercentage", label: "Turnover %", render: (v) => (v != null ? `${v}%` : "-") },

    { key: "active", label: "Status", render: (v) => <CBadge color={v ? "success" : "secondary"}>{v ? "Active" : "Inactive"}</CBadge> },
    {
        key: "actions",
        label: "Actions",
        render: (_, row) => (
            <>
                <CButton size="sm" color="info" onClick={() => onEdit(row)}>Edit</CButton>
