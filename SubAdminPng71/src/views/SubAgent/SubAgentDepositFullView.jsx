...
const DepositFullView = () => {
  ...
  const fetchDeposits = async () => {
    ...
    setError("");
    try {
      const res = await SubAdminServices.GetSubAgentPendingDepositList(filters);
  ...
  const handleStatusUpdate = async (transactionID, userId, actionType) => {
    ...
    setDepositLoading(true);
    try {
      const res = await SubAdminServices.updateDepositStatus(transactionID, userId, actionType);
        ...
        <CCardBody className="p-0">
          ...
          {depositLoading ? (
            ...
            <DataTable
              ...
              setSelectedRow={handleShowModal}
            />
          )}
        </CCardBody>
      </CCard>
