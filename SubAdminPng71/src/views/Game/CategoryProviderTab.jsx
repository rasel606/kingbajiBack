...
import { getCategoriesWithProviders,moveGamesToAnotherCategoryAndUpdateProviderList } from "../../service/gameManagementService";
...
export default function CategoryProviderTab() {
  ...
  const handleGameSelection = (game, checked) => {
    if (checked) {
      ...
      setMoveData(prev => ({
        ...prev,
        gamesToMove: prev.gamesToMove.filter(g => 
          !(g.g_code === game.g_code && g.p_code === game.p_code)
        )
      }));
    }
  };
  ...
  const handleMoveGames = async () => {
    if (!moveData.fromCategoryName || !moveData.toCategoryName || moveData.gamesToMove.length === 0) {
        ...
        <CCol md={8}>
          {selectedCategoryData ? (
            <CCard>
              <CCardHeader>
                <h5 className="mb-0">
                  {selectedCategoryData.category_name} - Games & Providers
                </h5>
              </CCardHeader>
              <CCardBody>
                {/* Providers List */}
                ...
                <CButton 
                  ...
                  disabled={!moveData.fromCategoryName || !moveData.toCategoryName || moveData.gamesToMove.length === 0}
                >
