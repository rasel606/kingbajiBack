...
const User = require("../Models/AffiliateModel");
...
console.log("referredbyCode",email, phone, password, countryCode, referredbyCode )
    ...
    const referredCode = Math.random().toString(36).substring(2, 13);
    const referrUserCode = Math.random().toString(36).substring(2, 13);
    if (referredbyCode) {
      const referredbyUser = await User.findOne({ referredCode: referredCode });
      const referrUser = await User.findOneAndDelete({ referredCode: referredCode });
      if(referrUser){
