...
cron.schedule('0 0 * * *', async ()=>{
  logger.info('Starting Daily VIP Update...');
  ...
  const bets = await BettingHistory.aggregate([
    ...
    { $group: { _id: "$member", totalTurnover: { $sum: "$turnover" } } }
  ]);

  for(const bet of bets){
    const updated = await vipService.updateUserVip(bet.member, bet.totalTurnover);
    logger.info(`Daily VIP Updated for ${bet._id}: Level ${updated.currentLevel}, Points ${updated.vipPoints}`);
  }
});

// Update every minute
cron.schedule('* * * * *', async ()=>{
  logger.info('Starting Minute VIP Update...');
  const users = await User.find({}, '_id');
  for(const u of users){
    const turnover = Math.random() * 1000; // Example: real turnover from real-time bets
    await vipService.updateUserVip(u._id, turnover);
  }
});
