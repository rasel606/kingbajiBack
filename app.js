...
const cookieHandler = require('./src/middleWare/cookieMiddleware');
const logger = require('./src/utils/logger');
const AdminAuth = require('./src/router/adminAurth');
const transactionRoutes = require('./src/router/transactionRoutes');
const subAdminRoutes = require('./src/router/subAdminRoutes');
const subAdminAuth = require('./src/router/subAdminAurth');
const dashboardRoutes = require('./src/router/dashboardRoutes');
const userRoutes = require('./src/router/userRoutes');
const gameRoutes = require('./src/router/gameRoutes');
const mainAdminRoutes = require('./src/router/mainAdminRoutes');
...
app.use('/api/subadmin/auth', subAdminAuth);
app.use('/api/subadmin/dashboard', subAdminDashboard);
app.use('/api/subadmin', subAdminRoutes);
...
// =======================
app.get('/api/socket/health', (req, res) => {
  res.json({ status: 'OK', socketEnabled: true, ...getConnectionStats(), timestamp: new Date() });
});

app.get('/api/socket/user/:userId', (req, res) => {
  const connections = getUserConnections(req.params.userId);
  res.json({ userId: req.params.userId, connections, totalConnections: connections.length, timestamp: new Date() });
});

app.get('/api/socket/room/:roomId', (req, res) => {
  const roomInfo = getRoomInfo(req.params.roomId);
  if (!roomInfo) return res.status(404).json({ error: 'Room not found', roomId: req.params.roomId, timestamp: new Date() });
  res.json(roomInfo);
});
...
app.get('/health', (req, res) => {
  res.json({
    ...
    memory: process.memoryUsage(),
    environment: process.env.NODE_ENV || 'development',
    socket: { enabled: true, ...getConnectionStats() }
  });
});
...
app.get('/health/detailed', (req, res) => {
  res.json({
    ...
    socket: { enabled: true, ...getConnectionStats() },
    database: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected',
    routes: ['/health', '/health/detailed', '/api/socket/health', '/api/socket/user/:userId', '/api/socket/room/:roomId']
  });
});
...
// 404 handler
app.use("*", (req, res) => {
  res.status(404).json({ status: "Fail", message: "Route not found", path: req.originalUrl, timestamp: new Date() });
});

// Global error handler
app.use((err, req, res, next) => {
  console.error('❌ Global error handler:', err);
  logger.error('Global Error Handler', { error: err.message, stack: err.stack, url: req.originalUrl, method: req.method, timestamp: new Date() });
  res.status(err.status || 500).json({ status: 'error', message: process.env.NODE_ENV === 'production' ? 'Internal server error' : err.message, ...(process.env.NODE_ENV !== 'production' && { stack: err.stack }), timestamp: new Date() });
});
